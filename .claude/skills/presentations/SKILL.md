...
| **from-brain** outline source (`POST /api/presentations/from-brain`) | Fast path: drafts a deck, report or page straight from the brain. Body: `{namespace, source, kind, locale, customer: {name, company, email}, style}` where `source` is one of `{type: "notes", note_ids: [...]}`, `{type: "search", query}`, `{type: "entity", entity}` (an entity id), `{type: "brain"}`. The result cites the notes/memories it used |
