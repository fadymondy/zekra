...
var noteToolDefs = []map[string]any{
	{
		...
		"inputSchema": obj(prop{
			...
			"description": prop{"type": "string", "description": "optional one-line summary shown under the title (max 500 characters)"},
	...
	{
		"name": "note_update",
		"description": "Update a note: any of title, description, body (replaces the whole body), tags, pinned, archived. " +
			"Pass the version you read to avoid overwriting someone else's edit (a mismatch returns the current copy).",
		"inputSchema": obj(prop{
			"id":          prop{"type": "string"},
			"title":       prop{"type": "string"},
			"description": prop{"type": "string", "description": "one-line summary (max 500 characters); \"\" clears it, omit to keep it"},
