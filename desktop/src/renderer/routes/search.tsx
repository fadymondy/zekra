    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <form onSubmit={submit} className="mx-auto flex w-full max-w-3xl items-center gap-2 px-6 pt-6 pb-3">
        <div className="relative flex min-w-0 flex-1 items-center">
          <SearchIcon className="pointer-events-none absolute start-3 size-4 text-grid-muted" />
          <Input
            ref={input}
            value={query}
            ...
            className="h-10 ps-9 pe-9 text-base"
          />
          {busy ? (
            <Loader2 aria-label={t("nav.updating")} className="absolute end-3 size-4 animate-spin text-grid-muted" />
          ) : query ? (
            <button
              type="button"
              ...
              className="absolute end-2.5 rounded-sm p-0.5 text-grid-muted hover:text-grid-fg"
            >
              <X className="size-4" />
            </button>
          ) : null}
        ...
        <div className="mx-auto w-full max-w-3xl px-6 pb-10">
          {!q ? (
            ...
            <div className="flex flex-col items-start gap-3 py-6">
              <p className="text-sm text-grid-danger">{t("searchx.failed")}</p>
            ...
            busy ? (
              ...
              <p className="py-4 text-sm text-grid-muted">{t("nav.liveHint")}</p>
            )
            ...
            <div className={cn("flex flex-col gap-6 pt-2 transition-opacity", busy && "opacity-60")}>
              <p className="grid-micro text-grid-muted">
                {t("nav.resultCount", { count: results.hits.length })}
                {error ? <span className="ms-2 text-grid-danger">· {t("searchx.failed")}</span> : null}
              </p>
              {groups.map((group) => {
                ...
                return (
                  <section key={group.namespace || "_"}>
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-medium text-grid-fg">{brainName(group.namespace)}</h2>
                      <span className="font-grid-mono text-[11px] text-grid-muted">{group.hits.length}</span>
                      {!route.ns && group.namespace ? (
                        <Button variant="ghost" size="xs" className="ms-auto text-grid-muted" onClick={() => setScope(group.namespace)}>
                          {t("nav.scopeBrain")}
                        </Button>
                      ) : null}
                    </div>
                      ...
                      {group.hits.map((hit) => (
                        <li key={hit.id}>
                          <button
                            ...
                            className="flex w-full items-start gap-3 px-3 py-3 text-start hover:bg-grid-soft focus-visible:outline-2 focus-visible:outline-grid-action"
                          >
                            <StickyNote className="mt-0.5 size-4 shrink-0 text-grid-muted" />
                            <span className="min-w-0 flex-1">
                              <span className="line-clamp-3 text-sm text-grid-body" style={{ unicodeBidi: "plaintext" }}>
                                {hit.content}
                              </span>
                              <span className="grid-micro mt-1 block text-grid-muted">{hitMeta(hit)}</span>
                            </span>
                          </button>
                        </li>
                      ))}
    ...
    <div className="flex flex-col gap-8 pt-2">
      <section>
        <div className="mb-2 flex items-center justify-between">
          <h2 className="grid-micro text-grid-muted">{t("nav.recent")}</h2>
          {recent.length ? (
            <Button variant="ghost" size="xs" className="text-grid-muted" onClick={onClearRecent}>
              {t("nav.clearRecent")}
            </Button>
          ) : null}
        </div>
        {recent.length ? (
            ...
            {recent.map((q) => (
              <li key={q} className="group flex items-center">
                <button
                  ...
                  onClick={() => onRecent(q)}
                  title={t("nav.rerun")}
                  className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5 text-start text-sm hover:bg-grid-soft"
                >
                  <Clock className="size-4 shrink-0 text-grid-muted" />
                ...
                <button
                  ...
                  className="me-2 rounded-sm p-1 text-grid-muted opacity-0 hover:text-grid-fg group-hover:opacity-100 focus-visible:opacity-100"
                >
                  <X className="size-3.5" />
                </button>
              </li>
          ...
          <p className="text-sm text-grid-muted">{t("searchx.noRecent")}</p>
        )}
      </section>

      <section>
        <h2 className="grid-micro mb-1 text-grid-muted">{t("searchx.browse")}</h2>
        <p className="mb-3 text-xs text-grid-muted">{t("searchx.browseHint")}</p>
        {!brains ? (
            ...
            {brains.map((b) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={cn(
                    "flex items-center gap-3 rounded-md border bg-grid-card p-3 text-start transition-colors hover:bg-grid-soft",
                    active ? "border-grid-gold/70 bg-grid-gold/10" : "border-line",
                  )}
                >
                  <BrainAvatar brain={b} token={token} size={36} />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-grid-fg">{b.displayName || b.namespace}</span>
                    <span className="block truncate font-grid-mono text-[11px] text-grid-muted">
                      {t("searchx.memories", { count: b.memories })}
                    </span>
                  </span>
                  <ChevronRight className={cn("size-4 shrink-0 text-grid-muted", isRtl && "rotate-180")} />
                </button>
              );
            })}
          </div>
