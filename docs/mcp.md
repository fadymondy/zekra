...
```toml
[mcp_servers.zekra]
url = "https://mcp.zekra.dev"
```

Then run `codex mcp login zekra` and complete the OAuth consent flow. For a local
self-hosted instance, use the stdio configuration generated by
`zekra mcp:install codex` instead.
...
| Codex | `codex mcp add zekra --url https://mcp.zekra.dev`, then `codex mcp login zekra` |
...
Clients without remote MCP support keep using the stdio server above.
