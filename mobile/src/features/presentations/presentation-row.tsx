...
export function PresentationRow({ item, onPress }: { item: Summary; onPress: () => void }) {
  const p = usePalette();
  const { t } = useI18n();
  const f = useFormat();
    ...
    <Row onPress={onPress}>
      ...
      <View style={styles.meta}>
        ...
        <View style={styles.stat}>
          <Link2 size={13} color={item.active_shares ? p.gold : p.muted} strokeWidth={1.8} />
          <AppText style={[styles.mono, { color: item.active_shares ? p.gold : p.muted }]}>{item.active_shares}</AppText>
        </View>
        <View style={styles.stat}>
